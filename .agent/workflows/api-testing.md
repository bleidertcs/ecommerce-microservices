...
1. **Obtener token de acceso desde Casdoor**

   ```bash
   # Reemplaza con tus credenciales de Casdoor
...
1. **Productos (Público - Rate Limited por IP)**
   ...
   curl -i http://localhost:8010/api/v1/products/1

   # Buscar productos
   curl -i "http://localhost:8010/api/v1/products?search=laptop"
   ```
...
2. **Usuarios (Protegido - Rate Limited por usuario)**
   ...
   curl -i -H "Authorization: Bearer $TOKEN" \
     http://localhost:8010/api/v1/users/profile
   ...
   # Obtener direcciones
   curl -i -H "Authorization: Bearer $TOKEN" \
     http://localhost:8010/api/v1/users/addresses
...
3. **Órdenes (Protegido - Circuit Breaker + Outbox)**
   ...
   curl -i -H "Authorization: Bearer $TOKEN" \
     http://localhost:8010/api/v1/orders/1
   ```
...
1. **Test de Rate Limiting (Público)**
   ...
   for i in {1..100}; do
     curl -s http://localhost:8010/api/v1/products > /dev/null
     echo "Request $i"
   done
   ```
...
2. **Test de Rate Limiting (Autenticado)**
   ...
   for i in {1..50}; do
     curl -s -H "Authorization: Bearer $TOKEN" \
       http://localhost:8010/api/v1/users/profile > /dev/null
     echo "Request $i"
   done
   ```
